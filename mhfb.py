...
# File           : mhfb.py                            #
# Author         : Mh Mamun   #
# Github         : https://github.com/mhprotechbd        #
# Facebook       : https://www.facebook.com/mmhasan.mamun   #
# Python version : 2.7                                #
#######################################################
...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m \ \/ / ____  /  |/  / _ )/ __/ ® \033[0m|| Created By Mh Mamun
 \033[0;96m  \  / /___/ / /|_/ / _  / _/     \033[0m|| Github.com/mhprotechbd
 \033[0;96m  /_/       /_/  /_/____/_/ \033[0;91mv2.0  \033[0m|| Facebook.com/mmhasan.mamun'''
# crack selesai
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		print '\n\n [%s!%s] oops you got no results :('%(M,N)
		exit()
...
def yayanxd():
	...
	if __cindy__ in ('open', 'Open', 'OPEN'):
		...
		print '%s *%s after switching to google chrome. klik %stitik tiga'%(B,N,H);time.sleep(2)
	...
	try:
		...
		zedd.close()
		print '\n\n %s*%s Wellcome %s%s%s'%(O,N,K,nama,N)
		time.sleep(2)
		print ' %s*%s please use this script properly, we are not responsible if this script is misused...'%(O,N)
		time.sleep(2)
...
def moch_yayan():
    ...
    print '___________________________________________________________'
    print '\n [%s1%s]. Dump id from friend'%(O,N)
    print ' [%s2%s]. Dump id from public friends'%(O,N)
    print ' [%s3%s]. Dump id from total followers'%(O,N)
    print ' [%s4%s]. Dump id from like posts'%(O,N)
    print ' [%s5%s]. Start crack'%(O,N)
    print ' [%s6%s]. Check fb account information'%(O,N)
    print ' [%s7%s]. View crack results'%(O,N)
    print ' [%s8%s]. Settings user agent'%(O,N)
    print ' [%s9%s]. Info %sscript'%(O,N,O)
...
def awokawokawokawokawokawokawokawokawokawokawokawok():
        yan = raw_input('\n [*] menu : ')
        if yan == '':
           print '\n %s[%s×%s] menu [%s%s%s] no, check the menu idiot!'%(N,M,N,M,yan,N);time.sleep(1);os.system('clear');moch_yayan()
        elif yan =='1':
                friends()
        elif yan =='2':
                publik()
        	...
        	jalan('\n note! ketik %suser%s if you want to get id of username'%(H,N));time.sleep(0.07)
        	cek_ingfo()
        elif yan =='7':
            ...
            if ask =="":
                ...
                try:
                    ...
                    print("\n \033[0;97m[\033[0;91m!\033[0;97m] You don't get results ok :(")
                    raw_input('\n  [ %sKEMBALI%s ] '%(O,N))
                    moch_yayan()
                ...
                try:
                    ...
                    print("\n \033[0;97m[\033[0;91m!\033[0;97m] You don't get cp results :(")
                    raw_input('\n  [ %sKEMBALI%s ] '%(O,N))
                    moch_yayan()
            else:
                moch_yayan()
        	...
        	info_tools()
                ...
                time.sleep(1)
                os.system('rm -rf __yayan__.txt')
                jalan ('\n %s[%s✓%s]%s successfully deleted token'%(N,H,N,H))
                time.sleep(2)
                exit()
        else:
            print '\n %s[%s×%s] menu [%s%s%s] no, check the menu idiot!'%(N,M,N,M,yan,N);time.sleep(1);os.system('clear');moch_yayan()
...
def teman():
    ...
    try:
        ...
        print ' [%s•%s] copy output file 👉 ( %s%s%s )'%(O,N,M,cin,N)
        print 50 * '-'
...
def publik():
    ...
    try:
        ...
        ys.close()
        jalan('\n\n %s[%s✓%s] successfully dump id from public'%(N,H,N))
        print ' [%s•%s] copy output file 👉 ( %s%s%s )'%(O,N,M,kntl,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N))
        moch_yayan()
    except (KeyError,IOError):
...
def postingan():
    ...
    try:
        ...
        print ' [%s•%s] copy output file 👉 ( %s%s%s )'%(O,N,M,ikeh,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N))
        moch_yayan()
    except (KeyError,IOError):
...
def cek_ingfo():
    ...
    try:
        ...
        if ppk in ('user', 'User', 'USER'):
        	jalan('\n [%s!%s] you will be redirected to the browser!'%(M,N));time.sleep(2)
    ...
    print ' [*] number : %s'%(nmrr);time.sleep(0.03)
    print ' [*] date of birth  : %s'%(ttll);time.sleep(0.03)
    print ' [*] gender  : %s'%(gndr);time.sleep(0.03)
    ...
    print ' [*] home town      : %s'%(dari);time.sleep(0.03)
    print ' [*] stay in     : %s'%(tigl);time.sleep(0.03)
    print ' [*] time zone    : %s'%(tzim);time.sleep(0.03)
    print ' [*] last updated : %s'%(uptd);time.sleep(0.03)
...
def ya_tanya_bapa_jangan_tanya_saya():
	ytbjts = raw_input('\n %s[%s?%s] choose : '%(N,O,N))
	if ytbjts == '':
		print '\n %s[%s×%s] Cant be empty Kentod'%(N,M,N)
		time.sleep(2)
		moch_yayan()
	elif ytbjts =='1':
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] no, dump id first, idiot!'%(N,M,N,M,self.apk,N)
            time.sleep(3)
            moch_yayan()

        ___yayanganteng___ = raw_input(' [%s?%s] do you want to use manual password? [Y/t]: '%(O,N))
        if ___yayanganteng___ in ('Y', 'y'):
            print '\n %s[%s!%s] use , (koma) for example separator : sandi123,sandi12345,dll. each word is at least 6 characters or more'%(N,M,N)
            while True:
